%% ODEs Generator - Question 3(c) PD Control with Reaction Wheels - WITH Magnetorquer

function  [xDOT] = ODEs_q3c_WithMagnetorquer(t,x,const,ATT,TORQ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
kh = const.kh;     % gain for magnetorquer
...
% Adding a magnetorquer for momentum management:
b_Fb = C_ba * b_Fa; 
gammadot_des = [0;0;0]; % desired reaction wheel speeds
m_Fb = -kh*Is*((b_Fb'*b_Fb)^-1)*crossm(b_Fb)*(x5 - gammadot_des); % magnetorquer dipole
tau_Mag = -1*crossm(b_Fb)*m_Fb;

x4_DOT =   inv(I) * (-1*crossm(x4)*I*x4 + tau_Bc + tau_CTRL - crossm(x4)*x5*Is + tau_Mag);
...
% Adding a magnetorquer for momentum management:
b_Fb = C_ba * b_Fa; % magnetic field vector resolved in Frame b
gammadot_des = [0;0;0]; % desired reaction wheel speeds
m_Fb = -kh*Is*((b_Fb'*b_Fb)^-1)*crossm(b_Fb)*(x5 - gammadot_des); % magnetorquer dipole
tau_Mag = -1*crossm(b_Fb)*m_Fb;


x6_DOT =   inv(I) * (-1*crossm(x6)*I*x6 + tau_Bc + tau_CTRL - crossm(x6)*x7*Is + tau_Mag);
