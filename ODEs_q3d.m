%% ODEs Generator - Question 3(d) Complementary Filter - Quaternion

function  [xDOT] = ODEs_q3d(t,x,const,TORQ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ke = const.ke;     % gain for Complementary Filter method
...
x5 = x(14:17); % quaternion estimate (Complementary Filter)
...
% Solve for ESTIMATED quaternion derivative using Complementary Filter
r_a = x1;
s1_a = -r_a;                      
s2_a = EarthMagField(r_a,t); 
s1_b = EarthSensorNoisy(s1_a,C_ba,t);
s2_b = MagnetometerNoisy(s2_a,C_ba,t);
C_ba_TRIAD = TRIAD(s1_a,s2_a,s1_b,s2_b); 
q_TRIAD = DCM2Quaternion(C_ba_TRIAD); % quaternion obtained from TRIAD
omega_est = RateGyroNoisy(x4,t); % angular velocity noisy measurements
q_err = [(x5(4)*eye(3)-crossm(x5(1:3)))    -x5(1:3);
                     x5(1:3)'               x5(4)  ]...
        * q_TRIAD;
     
x5_DOT = GammaQuaternion(x5)*(omega_est + ke*q_err(4)*q_err(1:3));
