%% ODEs Generator - Question 2 - Equation (2)

function  [xDOT] = ODEs_proj6(t,x,const,ATT,TORQ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
q_TRIAD = DCM2Quaternion(C_ba_TRIAD); 
omega_est = RateGyroNoisy(x4,t);
% tau_CTRL = -kp*x3(1:3)-kd*x4; % control torque using Eq(1) in 1(a)
tau_CTRL = -kp*q_TRIAD(1:3) -kd*omega_est; % control torque using Eq(2) in 1(b)
...
tau_CTRL = -kp*q_ba(1:3)-kd*x6; % control torque using Eq(1) in 1(a)
x6_DOT =   inv(I) * (-1*crossm(x6)*I*x6 + tau_Bc + tau_CTRL);
