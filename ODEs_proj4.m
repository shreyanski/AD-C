%% AEM 4305 PROJECT 4 - ODEs Generator

function  [xDOT] = ODEs_proj4(t,x,const,attitude,torque)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
x3 = x(7:9);   % epsilon vector of quaternion
x4 = x(10);    % eta value of quaternion
x5 = x(11:13); % angular velocity vector resolved in Frame b
...
q_DOT = GammaQuaternion([x3; x4]) * x5;
x3_DOT = q_DOT(1:3);
x4_DOT = q_DOT(4);

% Solve for angular velocity derivative using Euler's Law for Rotation
C_ba = Quaternion2DCM([x3; x4]); % obtain DCM from quaternion
...
x5_DOT =   inv(I) * (-1*crossm(x5)*I*x5 + tau_Bc);
